...
public abstract class Balance {
    ...
    public final void withdraw(Player player, int money) {
        ...
        YcoordCore.getInstance().getLogger().info(String.format("Игрок %s снял %d. Тип %s", player.getName(), money, this.getClass().getSimpleName()));
    }

    public final void deposit(Player player, int money) {
        subWithdraw(player, money);
        YcoordCore.getInstance().getLogger().info(String.format("Игроку %s внесено %d. Тип %s", player.getName(), money, this.getClass().getSimpleName()));
    }

    public final int get(Player player) {
        int amount = subGet(player);
        YcoordCore.getInstance().getLogger().info(String.format("Игрок %s извлек свой баланс %d. Тип %s", player.getName(), amount, this.getClass().getSimpleName()));
        return amount;
    }
